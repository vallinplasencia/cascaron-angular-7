...
@Component({
  ...
  styleUrls: ['./activo-editar.component.css']
})
export class ActivoEditarComponent implements OnInit {
      ...
      .subscribe((data: { itemData: ItemData<ActivoCampos | Errorr> }) => {
        switch (data.itemData.codigo) {
          case CodigoApp.OK: {
            ...
            this.nombRespSelecc = this.item.responsable ? this.item.responsable.nombre : '';
            this.categorias = campos.categorias;
            this.respIniItemData = campos.responsables;
            this.activoForm.setValue(
              {
                nombre: this.item.nombre,
                unidades: this.item.unidades,
                categoriaId: this.item.categoriaId,
                responsableId: this.item.responsableId,
                fechaAlta: this.item.fechaAlta,
                fechaBaja: this.item.fechaBaja,
                esPrincipal: this.item.esPrincipal,
              }
            );
            break;
          }
  ...
  onSubmit(formDirective: FormGroupDirective, evento) {
    ...
    this.snackBar.dismiss();
    let datosSalvar = this.activoForm.value;
          ...
          case CodigoApp.OK: {
            ...
            setTimeout(() => {
              this.snackBar.openFromComponent(SnackbarOkComponent, {
                data: ['Se guardó correctamente el activo:', act.nombre],
  ...
  getErrorMessage(campo: string) {
    ...
    switch (campo) {
      ...
      case 'fechaBaja': {
        if (this.fechaBaja.hasError('matDatepickerParse')) {
          return 'Proporcione una fecha con el formato DD/MM/AAAA.';
        }
        break;
      }
  ...
  comprobarFecha(campoFecha: string, evento) {
    ...
    if (!moment(target.value, 'DD/MM/YYYY').isValid()) {
      ...
      switch (campoFecha) {
        case 'fechaBaja':
          this.fechaBaja.patchValue(this.item.fechaBaja);
          break;
      }
    }
  }
  ...
  /**
   * Retorna el campo fechaBaja(FormControl)
   */
  get fechaBaja() {
    return this.activoForm.get('fechaBaja');
  }
  ...
  private hayCambiosForm() {
    ...
    return (this.item.nombre != this.nombre.value ||
      this.item.esPrincipal != this.esPrincipal.value ||
      this.item.fechaAlta != this.fechaAlta.value ||
      this.item.fechaBaja != this.fechaBaja.value ||
      this.item.unidades != this.unidades.value ||
      this.item.categoriaId != this.categoriaId.value ||
      this.item.responsableId != this.responsableId.value
    );
  }
